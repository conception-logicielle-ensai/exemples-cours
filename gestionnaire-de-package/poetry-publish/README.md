...
```
poetry install
poetry run python -m helloensaipoetry
```
...
```
pip install helloensaipoetry
python -m helloensaipoetry
```
...
5. `poetry publish` qui va envoyer le livrable sur `helloensaipoetry` par rapport a ce qui a été configuré sur le pyproject.toml et avec la valeur par défaut pypi
