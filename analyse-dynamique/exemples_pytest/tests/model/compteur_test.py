...
class TestCompteurComptageParLettre:
    ...
    def test_comptage_par_lettres_case_majuscules_minuscule_should_return_distinct_count(
        self,
    ):
        compteur = Compteur(["a", "A", "a", "b"])
        assert compteur.comptage_par_lettre == {"A": 1, "a": 2, "b": 1}

    def test_comptage_par_lettres_case_accent_num_symbole_should_return_empty_count(
        self,
    ):
