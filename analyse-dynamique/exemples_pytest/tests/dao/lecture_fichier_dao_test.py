...
def test_cas_simple_fichier_local(fichier_temporaire,tmp_path, dao:LectureFichierDAO):
    ...
    lignes = list(dao.lire_lignes(
        base_dir=tmp_path,
        chemin_fichier_str=PATH_FICHIER_DE_TEST
    ))
...
class TestLireLigneFichierLocal:
    ...
    """
    def test_cas_simple_fichier_local(self,fichier_temporaire,tmp_path, dao:LectureFichierDAO):
        """Cas simple : lecture d'un fichier local créé inline"""
...
class TestLireLigneFichierDistant:
    ...
    def test_lire_lignes_remote_file_mocking_requests(self, dao:LectureFichierDAO, mocker):
        """
        ...
        mock_response = mocker.Mock() # on peut "annuler les appels à différentes fonction"
        mock_response.raise_for_status = mocker.Mock() # comme celle qui controle le status HTTP de la requete
        ...
        lignes = list(dao.lire_lignes(
            base_dir="",
            chemin_fichier_str="https://api.example.com/data"
        ))
        
        # Assert
        assert lignes == ['{','"',"d","a","t","a",'"',':',' ','"','o','k','"','}']
